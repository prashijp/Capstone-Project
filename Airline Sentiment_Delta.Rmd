...
delta_sentiment <- delta_sentiment[,c(6:8,1:5)]
...
# Remove Stopwords. 
tweets_stopwords <- c(setdiff(stopwords('english'), c("r", "big","Delta",
 "jetblue","delta","airways","airlines","flight","pilot",
 "virgin","US airways","southwest","a","the","is","and")),"use", "see", 
 "used", "via", "amp","the","a","aa","aaaand","i","a","the",
 "flight","airlines","flights","airway","will", "cant","and","is","can","im")
...
#tweets_unibigram <- bind_cols(unigram_df, bigram_df)
#tweets_unibitrigram <- cbind(tweets_unibigram,trigram_df)
#tweets_model <- cbind(delta_sentiment,tweets_unibitrigram)

test_df  <- inner_join(delta_df, unigram_df, by = c("tweet_id", "text"))

tweets_unibigram <- inner_join(unigram_df, 
                             bigram_df,by = c("tweet_id", "text"))
dim(tweets_unibigram)
dim(tweets_unibitrigram)
dim(tweets_model)
 
tweets_unibitrigram <- full_join(tweets_unibigram,trigram_df,
                                 by = c("tweet_id","text"))

tweets_model <- full_join(delta_sentiment,tweets_unibitrigram,
                                 by = c("tweet_id","text")) 
